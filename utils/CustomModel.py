...
class FCNN(Model):
    ...
    def __init__(self, input_shape, batch_size = 10):
        ...
        self.input_layer = layers.Flatten(input_shape=input_shape, batch_size=self.batch_size)
        self.fc1 = layers.Dense(64*64, activation='sigmoid')
        self.mi = layers.Reshape([64,64])
        ...
        self.u3 = layers.UpSampling2D()
        self.d4 = layers.Conv2DTranspose(8, kernel_size=(3,3), activation='relu', padding='same')
        self.d5 = layers.Conv2D(1, kernel_size=(5,5), activation='sigmoid', padding='same')
        
        self.fd5 =  layers.Flatten(input_shape=input_shape, batch_size=self.batch_size)
        self.fc2 = layers.Dense(input_shape[0]*input_shape[1], activation= 'sigmoid')
        self.output_layer = layers.Reshape([input_shape[0],input_shape[1]])
